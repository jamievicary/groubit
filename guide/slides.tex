...
\uncover<6->{\node [anchor=north west] at (1,-2.5) {\includegraphics[height=4cm]{images/terminator}};}
}}
...
{
\picturefamily{
\node at (0,0) {\includegraphics[height=6cm]{images/schrodingers_cat_wanted}};
}
}
\end{frame}
...
\vspace{10pt}\pause
\textit{Step 2.} Alice, press random buttons, then press \inlineM to get your \textit{secret number}.
...
\vgap\pause
{\bf Alice's qubit has now been teleported to Charlie!}

%\vspace{10pt} \textit{Step 6.} Alice, press \inlineH then \inlineM. If the answer is 1, then Charlie press \inlineZ.

\vspace{10pt}\pause
\textit{Step 6.} Charlie, press \inlineM to discover {Alice's secret number.}
...
\vgap
We want to create \textit{large-scale} entanglement and superposition, with quantum effects taking place across the whole computer at once.
