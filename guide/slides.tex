...
\uncover<5->{\item build artificial intelligences.}
\end{itemize}
}
{
\picturefamily{
...
\uncover<6->{\node [anchor=north west] at (1,-2.5) {\includegraphics[height=4cm]{images/free/terminator}};}
}}
...
\splitpage{0.51}{
\vspace{5pt}
\uncover<3->{Atoms and electrons are some of the smallest things that exist.}

\vspace{10pt}
\uncover<4->{They obey the laws of \emph{quantum theory}--- mysterious, counterintuitive and surprising.}
}
{
\picturefamily{
...
\item \textit{Entanglement}---separate objects with a strong connection.

\item \textit{Teleportation}---jump straight from one place to another.
\end{itemize}
...
\vspace{20pt}
\textit{Simultaneously} dead \textit{and} alive ...

\vspace{20pt}
... but when we look, it's either dead \textit{or} alive.

\vspace{20pt}
`Looking' is called \textit{measurement}.
...
\vspace{15pt}
\textit{Step 3.} Alice, press \inlineH. {\bf The qubits are now entangled.}
...
\vspace{10pt}
%We are going to \textit{teleport} this secret number to Charlie!
