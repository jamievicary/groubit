...
\vgap
\uncover<10->{On each qubit, press \inlineCZ buttons\\at the same time.}
}{
...
\vspace{10pt}
\splitpage{0.55}
{\uncover<1->{Ordinary objects can't be in two places at once.}

\vgap
\uncover<2->{Quantum objects violate this simple rule,
\\
through the phenomenon of \textit{superposition}.}

\vgap
\uncover<3->{A famous example is \emph{Schr\"odinger's Cat}.}

\uncover<4->{\vgap It sits in a box, simultaneously dead \textit{and} alive.}
}
{
\picturefamily{
\uncover<3->{
\node at (0,0) {\includegraphics[height=5cm]{images/schrodingers_cat_wanted}};}
}
}


\vspace{7pt}
\uncover<5->{What happens when we look in the box? We see the cat either dead \textit{or} alive.}

\vgap
\uncover<6->{This is called \textit{measurement}. It destroys the superposition.}
\end{frame}
...
\vgap\pause
\step 3 Press \inlineM to measure at your qubit. The superposition has collapsed!
...
\vgap\pause
\step 1 Alice and Bob, both store 0 in your qubits.

\vgap\pause
\step 2 Alice and Bob, connect a cable and both press \inlineCZ. Remove the cable.

\vgap\pause
\step 3 Alice, press \inlineH.

\vgap\pause
{\bf The qubits are now entangled.}

\vgap\pause
\step 4 Alice and Bob, both press \inlineM. {\bf You should get the \textit{same result!}}
...
THIS IS TOO WEAK. ASK: HOW MANY BITS CAN YOU STORE IN A QUBIT?

IT'S CLEAR YOU CAN STORE ONE. DOES SUPERPOSITION HELP?

IN FACT A QUBIT CAN STORE TWO BITS.

THIS MEANS QUANTUM NETWORKS COULD TRANSFER INFORMATION VEYR EFFICIENTLY. USE `QUANTUM NETWORK' PICTURE. NO `CHALLENGE'.
...
\vgap
\step 1 Choose a number, either 0 or 1

\vgap
\step 2 Press \inlineH then \inlineM until your number is displayed

\vgap
\step 3 Pass the qubit to your friend.
...
\vgap
\step 1 Entangle your qubits. PREVIEW ENTANGLEMENT SLIDE BELOW.
...
\vgap
\step {10} Measure the qubit Alice gave you.\\This gives Alice's \textit{second number!}
\end{minipage}

\vgap
\textbf{We've encoded two bits in a single qubit!}
...
\vgap
\textit{How can this be possible?}
...
\draw [blue, ultra thick, ->] (-1.8,1.9) to [out=down, in=down, looseness=1.5] (1.8,1.9) to (1.8,5);
\end{tikzpicture}
\]
\end{minipage}
...
\vgap\pause
\step 1 Alice and Bob, entangle your qubits! REPEAT FROM BEFORE
...
\item Teleportation
\end{itemize}
But \textit{when} will quantum computers arrive?
\vspace{-10pt}
\end{minipage}
...
\vspace{20pt}
\uncover<2->{Just last month, IBM launched a 16-qubit quantum computer you can use over the internet.}

\vgap
\uncover<3->{Google plans to release a 49-qubit machine by the end of the year.}

\vgap
\uncover<4->{\textit{The quantum revolution is about to begin!}}
}
{
\picturefamily{
